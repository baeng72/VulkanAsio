...
void VertexBufferBuilder::build(Vulkan::Buffer& buffer_, std::vector<uint32_t>& vertexLocations,void**ptrOut) {
	...
	if (isMapped)
		*ptrOut = Vulkan::mapBuffer(device, buffer_);
	if (vertexSizes.size()) {
		if (isMapped) {
...
void IndexBufferBuilder::build(Vulkan::Buffer& buffer_, std::vector<uint32_t>& indexLocations,void**ptrOut) {
...
void TextureLoader::loadCubeMap(std::vector<uint8_t*>& pixelArray, uint32_t width, uint32_t height, bool enableLod, Vulkan::Texture& image) {
...
void PipelineLayoutBuilder::build(VkPipelineLayout& pipelineLayout) {
	pipelineLayout = Vulkan::initPipelineLayout(device, descriptorSetLayouts,pushConstants);
}
